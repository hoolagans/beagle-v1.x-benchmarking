...
public class Parse : AttributeList
{
    ...
    public void ParseAttributeName()
    {
        ...
        if (eof) throw new EOFException("Unexpected EOF while expecting a tag to close its angle bracket");
        EatWhiteSpace();
    }
        ...
        if (GetCurrentChar() == '=')
        {
            ...
            if ((GetCurrentChar() == '\'') ||
                ...
                while (GetCurrentChar() != m_parseDelim)
                {
                    ...
                    m_idx++;
                    if (m_idx >= Source.Length) throw new BrowserEmulatorException($"Unexpected EOF while expecting a closing quote ({m_parseDelim})");
                }
                m_idx++;
            }
            else
