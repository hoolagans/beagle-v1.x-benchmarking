...
public abstract class ApiModelForAnyEntity : IViewModelForAnyEntity
{
    ...
    public virtual async Task<T> MapToCustomAsync<T>(T other)
    {
        if (other == null || (long)other.PropertyGet("Id")! != Id)
        {
            ...
            else otherObj = await RepoFactory.CreateForRuntimeType(type).GetIEntityByIdAsync(Id).ConfigureAwait(false);

            return (T)await this.MapToAsync(otherObj, type).ConfigureAwait(false);
        }
        return await this.MapToCustomBaseAsync(other).ConfigureAwait(false);
    }
    #endregion
